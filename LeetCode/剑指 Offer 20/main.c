//我直接把65题的代码复制粘贴过来加上对E的判定就过了，哈哈哈
bool isNumber(char * s){
    //point用来记录小数点是否出现,sign_e用来记录e或者E是否出现
    //num用来记录e或者E之前出现数字的个数，num1用来记录e或者E之后出现数字的个数
    ...
    for(;i<=j;i++){
        if(s[i]==' ')//字符串中出现空格是错误的
            return false;
        if((s[i]>='a'&&s[i]<='z'&&s[i]!='e')||(s[i]>='A'&&s[i]<='Z' && s[i] != 'E'))//出现了除e或者E以外的字母是错误的
            return false;
        if(s[i]=='-'||s[i]=='+')//字符串中最多可以出现两次这种符号
            return false;//一次在去空格后字符串的头部，另一次是紧跟着出现在e或者E的后面，除了这两种情况再出现就是错误的
 if((s[i]=='.'&&point==1)||(s[i]=='.'&&sign_e==1)||(s[i]=='.'&&num==0&&(s[i+1]<'0'||s[i+1]>'9')))
        ...
        if((s[i]>='0'&&s[i]<='9')&&sign_e==1)
            num1++;
        if(s[i]=='e' || s[i] == 'E'){
            if(num==0)//e或者E之前如果没数字是错误的
                return false;
            if(sign_e==1)//e或者E多次出现是错误的
                return false;
            sign_e=1;
            if(s[i+1]=='-'||s[i+1]=='+'){
                i=i+1;
    ...
    if(sign_e==1&&num1==0)//e或者E之后没数字是错误的
        return false;
    return true;
}
